...
void vtkSlicerMVModellerLogic::closeMVOpening(vtkMRMLNode * node)
{
    ...
    nodeToPolyCardinalSpline( vtkMRMLMarkupsFiducialNode::SafeDownCast(node), "MVProfileSpline", true );
	generateOpeningPlanes();
}

//---------------------------------------------------------------------------
void vtkSlicerMVModellerLogic::nodeToPolyCardinalSpline(vtkMRMLMarkupsFiducialNode* sourceNode, const char* modelName, bool closed)
{
    if (!sourceNode)
    {
        vtkWarningMacro("Bad node pointer" << endl);
    ...
    model->SetAndObservePolyData(outputPoly.GetPointer());
    model->Modified();
...
{
	vtkSmartPointer<vtkMRMLModelNode> profile = vtkMRMLModelNode::SafeDownCast(this->GetMRMLScene()->GetFirstNodeByName("MVProfileSpline"));
	...
	vtkSmartPointer<vtkPoints> points = profile->GetPolyData()->GetPoints();
	...
	for (int i = 0; i < nPoints - 1; i += planeStep)
	{
...
void vtkSlicerMVModellerLogic::selectMVPlane(const int & i)
{
    ...
    {
        vtkSmartPointer<vtkMRMLModelNode> model = vtkMRMLModelNode::SafeDownCast(planes->GetItemAsObject(i));
        vtkSmartPointer<vtkPoints> points = model->GetPolyData()->GetPoints();

        vtkNew<vtkPlaneSource> plane;
        plane->SetOrigin(points->GetPoint(0));
        plane->SetPoint1(points->GetPoint(1));
